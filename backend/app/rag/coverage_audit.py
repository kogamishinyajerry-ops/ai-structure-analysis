...
def audit_coverage(repo_root: Path) -> CoverageReport:
    """Walk disk + registry; produce a structured CoverageReport. Never raises."""
    ...
    def _bucket(name: str, expected: list[str], covered: set[str]) -> CoverageBucket:
        exp_set = set(expected)
        # Restrict the "covered" comparison to the same kind of file —
        # otherwise GS readmes would appear as extras when comparing
        # against ADR expected, and vice versa.
        relevant_covered = {p for p in covered if p in exp_set}
        missing = sorted(exp_set - covered)
        return CoverageBucket(
            name=name,
            expected_files=tuple(sorted(expected)),
            covered_files=tuple(sorted(relevant_covered)),
            missing_files=tuple(missing),
            # Extras = paths the source ingested that DON'T match any expected
            # (within this bucket's scope). For ADR bucket: a file like
            # 'docs/failure_patterns/FP-XXX.md' is OUTSIDE this bucket; it
            # belongs to the FP bucket. So extras must be filtered.
            extra_files=tuple(),  # populated below by a single pass over the source
        )

    # Now build the per-source extras: paths the source emitted that are
    # not in ANY of its bucket's expected sets.
