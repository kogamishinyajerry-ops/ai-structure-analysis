...
- **FF-07**: CI 实现 commit trailer presence + claim-id 格式校验（HF5 detection automation）— gated on completion (no calendar deadline per AR-2026-05-03-001). **Hard prerequisite (Phase 2)**: must merge before Phase 2 Web Console activation Gate is opened, AND before Calibration Mode close-out predicate evaluates true. **Discipline-only sub-clause (audit, not enforced — per Codex R2 LOW #4)**: PRs SHOULD NOT claim "HF5 automation enforced" in their body until FF-07 lands; this is a discipline norm enforceable only via human review of PR bodies and post-merge audit (no CI / status-check / trailer-gate exists for this claim today). A future ADR may add an ADR-013 status-check that mechanically validates this; until then, treat any "HF5 automation enforced" claim in a PR body as a candidate for `reports/codex_tool_reports/` audit, not for CI rejection.
- **FF-08**: `golden_samples/<id>` registry schema 校验（HF3 detection automation）— gated on completion (no calendar deadline per AR-2026-05-03-001). **Hard prerequisite (Phase 2)**: must merge before Phase 2 Web Console activation Gate is opened. **Discipline-only sub-clause (audit, not enforced — per Codex R2 LOW #4)**: PRs SHOULD NOT claim "HF3 automation enforced" in their body until FF-08 lands; same discipline / audit semantics as FF-07's HF5 sub-clause above.
...
- **Calibration window** (per AR-2026-05-03-001): **task-completion-driven, no calendar end date**. The window opens at 2026-04-25 (this ADR's first ratification) and **closes when the close-out predicate evaluates true**.
  ...
  **Defined event (used throughout this ADR)**: `Phase_2_Gate_queued` ≜ "Phase 2 Web Console activation has been listed on the T0 Gate queue (i.e., a request for T0 to open the Phase 2 activation Gate has been entered into the Decisions DB Pending list)". This is the single canonical event term; "imminent", "queued", "opened" must NOT be used as separate states elsewhere in this ADR.
