...
- **HF1.7b** `golden_samples/*-candidate/` — WRITABLE per the FM-04a Tier 1 candidate binding constraint (Phase 13 D carve-out per AR-2026-05-16-001). The `*-candidate` suffix is unambiguous + automatable; the path-guard's `_is_candidate_carveout` helper recognizes the carve-out without override. Defense in depth: the carve-out helper REJECTS `GS-NNN-candidate` first-segments (signed-registry pattern wins over carve-out suffix), so a hypothetical name collision still trips HF1.7a hard-stop.
