...
- **Status:** Accepted (amended 2026-04-25 per AR-2026-04-25-001; amended 2026-05-03 per AR-2026-05-03-001; amended 2026-05-06 per AR-2026-05-06-001; amended 2026-05-16 per AR-2026-05-16-001)
- **Decider:** Human owner — Codex-primary workflow ratified by ENG-32 pilot and local Claude Opus audit
- **Date:** 2026-04-25 (R5 APPROVE), amended 2026-04-25, 2026-05-03, 2026-05-06, and 2026-05-16
...
- **Amendment cycles:**
  ...
  - **2026-05-16 AR-2026-05-16-001** — FM-04a Phase 13 D ratification: HF1.7 (`golden_samples/**` — 全部只读) splits into **HF1.7a** (`golden_samples/<signed-registry>/` — signed-registry whole-directory read-only) + **HF1.7b** (`golden_samples/*-candidate/` — WRITABLE per the FM-04a Tier 1 candidate binding constraint). Motivation: Phase 12 C (commit `75a21cd`) and Phase 13 C (commit `710176a`) each had to land `*-candidate` fixture writes via `HF1_GUARD_OVERRIDE`, every invocation logged in `reports/hf_audit.md`. The override-abuse risk scales with the count of invocations, and the override-abuse mitigation surface (`*-candidate` suffix) is small + automatable — a 1-line regex change closes the override path entirely. The amendment lands the regex carve-out in `scripts/hf1_path_guard.py` (`_is_candidate_carveout` helper) so `*-candidate` writes pass without override. Signed-registry shape `^GS-\d{3}$` remains hard-stop (defense in depth: the carve-out helper REJECTS BOTH the canonical fullmatch `^GS-\d{3}$` AND the prefix shape `^GS-\d{3}-` — Phase 13 E slice-D MEDIUM-1 closure — so `GS-NNN-candidate` names are also hard-stop, not carve-out). Companion edits in the same amendment cycle: `reports/hf_audit.md` rolls over to window-001 (the Phase 12 C + Phase 13 C entries become historical context under `reports/archive/hf_audit_window-000.md`); the path-guard self-protection clause (HF1.8) is unchanged.
...
- **HF1.7a** `golden_samples/<signed-registry>/` — signed-registry whole-directory read-only (the `^GS-\d{3}$` shape is sealed FM-04b P8 packet surface). Hard-stop; no carve-out. (Amended from "HF1.7 — `golden_samples/**` — 全部只读" per AR-2026-05-16-001 §HF1 split.)
- **HF1.7b** `golden_samples/*-candidate/` — WRITABLE per the FM-04a Tier 1 candidate binding constraint (Phase 13 D carve-out per AR-2026-05-16-001; Phase 13 E MEDIUM-1 closure tightens the defense-in-depth pin). The `*-candidate` suffix is unambiguous + automatable; the path-guard's `_is_candidate_carveout` helper recognizes the carve-out without override. Defense in depth: the helper REJECTS BOTH the canonical signed-registry fullmatch `^GS-\d{3}$` AND the signed-registry prefix shape `^GS-\d{3}-` — so `GS-101-candidate` (signed-registry prefix with `-candidate` suffix) is ALSO hard-stop, not carve-out. The signed-registry pattern wins over the carve-out suffix in both shapes.
...
| **HF1 (PR-protected zone)** | branch protection (per ADR-013) + mandatory Codex M1 trigger (per amended §T2) | ADR-013 landed; branch protection requires status checks, linear history, and conversation resolution. Codex M1 remains mandatory for governance text. |
...
| HF3 (sample registry) | `scripts/validate_golden_samples.py` validates signed `GS-###` registry metadata; `.github/workflows/golden-samples-validation.yml` runs it from trusted `main` | FF-08 landed via PR #131; branch protection now requires `golden-samples-validation` |
...
**Tracking tasks (FF-06/07/08 landed; FF-09 remaining):**
...
- **FF-08** **done** — signed golden-sample registry validation landed in PR #131 and branch protection requires `golden-samples-validation`.
...
In summary: **HF1 pre-commit enforcement landed in FF-06**. HF5 is enforced by FF-07. HF3 is enforced by FF-08. Branch protection per ADR-013 requires `lint-and-test (3.11)`, `calibration-cap-check`, `trailer-check`, and `golden-samples-validation`. HF2 is calibration-mode honor-system pending §Calibration Mode close-out (no calendar deadline per AR-2026-05-03-001), then hard-stop. HF4 is partial automation + manual reconciliation.
...
6. **Residual enforcement maturity gap** — FF-06/07/08 closed the major HF1/HF5/HF3 automation gaps. Remaining maturity gaps are narrower: HF2 still depends on calibration-window discipline until close-out, and HF4 still depends on partial Notion sync automation plus manual reconciliation. Do not over-claim those as fully automated.
...
The original ADR-011 gap list is retained for audit history. Items marked
resolved below have since landed; remaining items still need future ADRs.

1. **`main` branch protection** — resolved by ADR-013 and `scripts/apply_branch_protection.sh`. Required checks, linear history, no force-pushes/deletions, and conversation resolution are active; `enforce_admins=false` remains a deliberate emergency-override tradeoff.
2. **PR review state machine** — partially resolved by ADR-013 plus the Codex-primary / Claude-audited workflow. GitHub does not require human reviews in this solo-maintainer setup, but mandatory review evidence and conversation resolution are policy requirements.
...
- FF-09 synchronizes `README.md` and `docs/governance/` with ADR-011 / ADR-012 / ADR-013. The README remains a quick-reference; this ADR remains canonical when they conflict.
