    ...
    print(
        "\n".join(
            [
                ...
                "  cross-check for the plate-with-hole candidate, then re-run this renderer:",
                "",
                "    .venv/bin/python scripts/cross_check_phase21a.py --skip cantilever",
                "",
                "  That CLI runs the real solver into a TemporaryDirectory. Copy the produced",
                "  plate_kirsch_xcheck.frd to the path above (or re-point FRD), e.g.:",
                f"    mkdir -p {Path(FRD).parent}",
                f"    cp <tmpdir>/plate-with-hole-candidate/plate_kirsch_xcheck.frd {FRD}",
            ]
        ),
        file=sys.stderr,
    )
