...
ADR-012 BLOCKING ceiling 30% 的本意：**自评估上限不能高估**。W7 期间所有 5 个 PR 都按 30% 自评。**4 个 PR (W7b/W7d/W7f/W7-tools) 走完了 first-round CHANGES_REQUIRED → 修复 → APPROVE 的循环；PR #94 (W7c) 是例外**——first-round 表面看像 APPROVE_WITH_NITS（follow-up commit 标题为 "PR #94 R1 nits"），但因为 #92-#95 的 round 日志当时未保存，"nit 还是 CHANGES_REQUIRED" 的精确边界不可考；§1.1 已经做了 floor-only 的诚实表述。下面的"指标"表也按这个 floor 重计。

| 指标 | 数值（floor / 估算） |
|---|---|
| 总 Codex 轮次（5 PR） | ≥15（floor，含 PR #96 完整 4 轮 + 其余 4 PR ≥2-≥4 轮） |
| 平均每 PR 轮次 | ≥3 |
| Tokens 累计估算 | **≥608K（floor，仅 PR #96 R1-R4 实测）+ #92/#93/#94/#95 extrapolated**。PR #96 实测 4 轮: R1 304,776 / R2 79,364 / R3 145,738 / R4 78,502 = **608,380**（来自 `reports/codex_tool_reports/pr96_w7tools_r{1..4}.log`）。其余 4 PR 的 round 日志当时未保存，假设每轮规模与 #96 同量级且每 PR ≥2 轮，则总量 ≈ 1.0–1.5M tokens 范围（不计入 floor）。 |
