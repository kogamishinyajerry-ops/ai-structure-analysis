...
def test_iter_metadata_shape(tmp_path):
    repo = _make_synth(tmp_path)
    docs = list(iter_gs_theory_documents(repo))
    readme_doc = next(d for d in docs if d.doc_id == "GS-X:README")
    ...
    theory_doc = next(d for d in docs if d.doc_id == "GS-X:cantilever_theory")
...
def test_iter_skips_empty_readme(tmp_path):
    repo = _make_synth(tmp_path)
    docs = list(iter_gs_theory_documents(repo))
    assert not any(d.doc_id == "GS-Z:README" for d in docs)
...
def test_iter_ignores_non_gs_dirs(tmp_path):
    repo = _make_synth(tmp_path)
    docs = list(iter_gs_theory_documents(repo))
    assert all(d.doc_id.startswith("GS-") for d in docs)
...
def test_main_lists_docs(tmp_path, capsys):
    ...
    assert "GS-X:README" in out
    assert "GS-X:cantilever_theory" in out
