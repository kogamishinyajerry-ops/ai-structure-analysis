...
def test_audit_coverage_missing_when_iter_fn_skips_a_file(tmp_path, monkeypatch):
    ...
    # source-4 hardening — the bare `ADR-100` form predates that change.
    ...
    for label, fn in original:
        if label == "project-adr-fp":
            ...
            def _filtered(root, _orig=fn):
                for d in _orig(root):
                    if d.doc_id != "project-adr-fp:ADR-100":
                        yield d
...
# ---------------------------------------------------------------------------
# Pre-emptive R2 hardening (lifted from PR #59 + #60 + #61 + #62 patterns):
# - rc=2 contract via _UsageError (not plain SystemExit("msg"))
# - corpus iteration ValueError/OSError translated to rc=2 + single line
# ---------------------------------------------------------------------------
...
def test_main_corpus_value_error_translates_to_rc_2(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(cmod, "ALL_SOURCES", [("project-adr-fp", bad_iter)])

    rc = cmod.main(["coverage_audit.py", "--root", str(tmp_path)])
    err = capsys.readouterr().err
    assert rc == 2
    assert "corpus iteration failed" in err
    assert "simulated duplicate doc_id" in err
    assert err.count("[coverage-audit]") == 1
