...
def test_generate_naca_points():
    ...
    # The middle of the array is the leading edge due to trailing -> upper -> leading -> lower -> trailing
...
def test_generate_geometry_with_freecad_mocks(tmp_path):
    ...
    with patch.dict("sys.modules", {"FreeCAD": mock_freecad, "Part": mock_part}):
        with patch("tools.freecad_driver.FREECAD_AVAILABLE", True):
            ...
            assert data[0]["fixed_root"] == ["Face1"]
            assert data[0]["skin"] == ["Face2"]
            assert data[0]["tip"] == ["Face3"]
