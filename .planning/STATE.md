...
> **Stamp:** `FF-08-in-review-2026-05-06 · main=f79aadf · pr=131`
> **Last updated:** 2026-05-06 (after PR #131 opened from `codex/ff-08-gs-registry-validation`)
...
| Phase 1.5 — Foundation-Freeze (post-pivot) | 🟡 Governance baseline amended by WF-00 / ENG-32: Codex-primary + Linear work-control + Claude Opus audit. ENG-33 AERON L0 protocol salvage done via #128. FF-07 done via #129 and branch protection now requires `trailer-check`; FF-08 Codex-owned replacement is in review via #131; FF-09 remains next. |
...
| FF-08 — `golden_samples/<id>` registry schema validation (HF3) | 🟡 In review · Codex-owned replacement PR #131 · Claude Opus APPROVE · local verification green | #131 | `26a41b4` (PR branch tip) | Adds trusted signed-registry validator, always-on `golden-samples-validation` workflow, ADR-011/013 sync, and branch-protection context update. Old #28 remains open until #131 lands, then should be closed as superseded. |
...
`main == origin/main == f79aadf` (post FF-07 closeout/state PR #130 merge, 2026-05-06).
...
| PR | Branch | Status | Notes |
|----|--------|--------|-------|
| #131 | `codex/ff-08-gs-registry-validation` | OPEN · FF-08 / ENG-17 | Codex-owned replacement for old #28. Adds HF3 validator + always-on `golden-samples-validation` workflow + branch-protection context update. |
...
| #28 | `feature/AI-FEA-FF-08-gs-registry` | OPEN · superseded by Codex replacement #131 once #131 lands |
...
1. **FF-08 / FF-09**: FF-08 is in review via Codex-owned PR #131; after #131 lands, rerun `scripts/apply_branch_protection.sh`, verify `golden-samples-validation` is required on the next PR, and close old #28 as superseded. FF-09 remains next. Gated on completion alone — **no date deadline applies** per project policy.
